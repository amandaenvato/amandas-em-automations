...
4. The task will be appended to `files/tasks/pending.jsonl` in the repository root

## Task Format

Tasks are written to `files/tasks/pending.jsonl` in JSONL format (one JSON object per line). The task manager reads only from `pending.jsonl` (completed tasks are stored in `completed.jsonl` and are not displayed):
```json
